...
void printBoard(int color1, int color2, char board[8][8])
{
...
bool checkExists(char board[8][8], bool isKingWhite, bool &isWhiteKingMoved, bool &isBlackKingMoved, bool &isWhiteRookAMoved, bool &isWhiteRookHMoved, bool &isBlackRookAMoved, bool &isBlackRookHMoved) {
    std::pair<int, int> king;
    // Locate the king
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            if ((board[i][j] == 'K' && isKingWhite) || (board[i][j] == 'k' && !isKingWhite)) {
...
bool canKingMove(bool white, char board[8][8], bool &isWhiteKingMoved, bool &isBlackKingMoved, bool &isWhiteRookAMoved, bool &isWhiteRookHMoved, bool &isBlackRookAMoved, bool &isBlackRookHMoved) {
...
bool solvesCheck(bool white, char board[8][8], std::pair<int, int> old, std::pair<int, int> newC, bool &isWhiteKingMoved, bool &isBlackKingMoved,
                bool &isWhiteRookAMoved, bool &isWhiteRookHMoved,
                bool &isBlackRookAMoved, bool &isBlackRookHMoved)
{
    char tempBoard[8][8];
...
int main() {
    ...
    while (true) {
        std::cout << std::endl;
        printBoard(WHITE_SQUARE, BLACK_SQUARE, board);


        // In the main loop, check for checkmate or stalemate:
        if (checkExists(board, white_turn, isWhiteKingMoved, isBlackKingMoved, isWhiteRookAMoved, isWhiteRookHMoved, isBlackRookAMoved, isBlackRookHMoved)) {
            if (canKingMove(white_turn, board, isWhiteKingMoved, isBlackKingMoved, isWhiteRookAMoved, isWhiteRookHMoved, isBlackRookAMoved, isBlackRookHMoved) || canOthersSaveKing(white_turn, board, isWhiteKingMoved, isBlackKingMoved, isWhiteRookAMoved, isWhiteRookHMoved, isBlackRookAMoved, isBlackRookHMoved)) {
        ...
        std::pair<int, int> old = chessboardToArrCords(start_location);
        std::pair<int, int> newC = chessboardToArrCords(destination);

        // Ensure valid turn
        if (std::isupper(board[old.first][old.second]) == white_turn) {
            // Check if move is valid (including castling)
            if (verifyMove(old, newC, board,
                           isWhiteKingMoved, isBlackKingMoved,
                           isWhiteRookAMoved, isWhiteRookHMoved,
